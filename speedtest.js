 ...
 */
var test; // Variabile globale per il test

function startTest() {
    test = new Speedtest(); // Istanziazione automatica del test di Librespeed
    test.onupdate = function(data) {
        // Aggiorna i risultati durante il test
        document.getElementById("dlText").textContent = (data.dlStatus === "" ? "..." : data.dlStatus); // Velocità di download
        document.getElementById("ulText").textContent = (data.ulStatus === "" ? "..." : data.ulStatus); // Velocità di upload
        // Inserisci i risultati nei campi input
        document.getElementById("downloadResult").value = data.dlStatus;
        document.getElementById("uploadResult").value = data.ulStatus;
    };
